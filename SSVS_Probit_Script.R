#
#	EST-46111:	Fundamentos de Estadistica (Maestria en Ciencia de Datos)
...
rm(list = ls())
setwd("C:/JCMO.Trabajo/@Mis.Cursos/2016-B_Fundamentos Estadistica/Datos/SSVS_Probit/")
path.data = 'C:/JCMO.Trabajo/@Mis.Cursos/2016-B_Fundamentos Estadistica/Datos/SSVS_Probit/'
...
#		Datos
#	----------------------------------------------------
dde <- read.csv(paste(path.data,'SSVS_Probit_Data.csv',sep = ""), header = TRUE, sep = ",", quote="\"", dec=".", fill = TRUE)

#	Inputs
# x = dde 	# dosis
# Y = 0/1 	# variable indicadora sobre embarazo premanuturo
...
for(gt in 1:G){
  ...
  #	Output
  write(t(beta),file=paste(path.data,'Output/SSVS_Probit_beta.out',sep = ""), ncol=7, append=T)
  print(c(gt,round(beta*100)/100))
}
#	FIN: Gibbs sampler

#	Trace-plots para beta[1] y beta[2]
pdf(paste(path.data,'Output/SSVS_Probit_beta_traces.pdf',sep = ""),width=7,height=5)
par(mfrow=c(2,1))
beta_out<- matrix(scan(paste(path.data,'Output/SSVS_Probit_beta.out',sep = "")), ncol=7, byrow=T)
...
pdf(paste(path.data,'Output/SSVS_Probit_beta_marginal_slope.pdf',sep = ""),width=7,height=5)
...
pdf(paste(path.data,'Output/SSVS_Probit_beta_doseresponse.pdf',sep = ""),width=7,height=5)
par(mfrow=c(1,1))
par(mar=c(5,5,5,5))
...
plot(xtrue,post[,1],xlab="Serum DDE (mg/L)",ylab="Pr(Nacimiento prematuro)",cex=1.2,
...
write.csv(t(table1),file=paste(path.data,'Output/SSVS_Probit_beta_summary.csv',sep = ""))
...
for(gt in 1:G){
  ...
  #	Output
  write(t(beta),file=paste(path.data,'Output/SSVS_Probit_beta_ss.out',sep = ""), ncol=7, append=T)
  print(c(gt,round(beta*100)/100))
}
#	FIN: Stochastic search Gibbs sampler

#	Trace-plots para beta[1] y beta[2]
beta<- matrix(scan(paste(path.data,'Output/SSVS_Probit_beta_ss.out',sep = "")), ncol=7, byrow=T)
# 2^p = Numero de posibles modelos
# En este caso, 2^p = 2^7 = 128

pdf(paste(path.data,'Output/SSVS_Probit_beta_traces_ss.pdf',sep = ""),width=7,height=5)
...
pdf(paste(path.data,'Output/SSVS_Probit_beta_doseresponse_ss.pdf',sep = ""),width=7,height=5)
par(mfrow=c(1,1))
par(mar=c(5,5,5,5))
xg<- seq(-2,2,length=100)  	# Grid
...
table1<- round(table1*100)/100
write(t(table1),file=paste(path.data,'Output/SSVS_Probit_beta_summary_ss.csv',sep = ""),ncol=6)
...
write(t(table2),file=paste(path.data,'Output/SSVS_Probit_beta_pmodel_ss.csv',sep = ""),ncol=8)

#	Save image
save.image(file=paste(path.data,'Output/SSVS_Probit_Output.Rdata',sep = ""))

#
#	-- FIN: SSVS_Probit_Script.R --